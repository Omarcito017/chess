...
class Board:
    ...
    def check_position_format(self, pos: str) -> None:
    ...
    """
        ...
        if pos[0] not in self.columns.keys():
    ...
    def position_to_idxs(self, pos: str) -> Tuple[int, int]:
        ...
        """
        check_position_format(pos)
        i, j = int(pos[1]) - 1, self.columns[pos[0]]
        return (i, j)
    ...
    def valid_moves(self, pos: str) -> List[str]:
        ...
        if isinstance(curr_piece, Pawn):
            moves = self.pawn_valid_moves(pos)
        return sorted(moves)

    def pawn_valid_moves(self, pos: str) -> List[str]:
